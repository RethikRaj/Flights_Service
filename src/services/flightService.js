...
async function getFlight(id){
    try {
        ...
        throw new AppError(["Error while fetching data of the flight"], StatusCodes.INTERNAL_SERVER_ERROR);
    }
}

async function updateSeats(data){
    try {
        const flight = await flightRepository.updateSeats(data.flightId, data.numberOfSeats, data.decrease);
        return flight;
    } catch (error) {
        console.log(error);
        if(error.statusCode == StatusCodes.NOT_FOUND){
